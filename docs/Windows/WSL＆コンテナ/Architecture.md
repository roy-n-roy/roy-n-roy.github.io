...
| --------------------- | ------------------ | -------- | ---------- |
| バージョン1<br>(WSL1) | システムコール変換 | メモリリソースの消費が少ない | 命令変換のオーバーヘッドが発生し、<br>CPUリソースの消費が増える<br>一部のシステムコール命令が提供されない<br>ディスクアクセス速度が遅い |
| バージョン2<br>(WSL2) | 仮想マシン         | システムコールの完全互換<br> | ファイルシステムキャッシュなどでのメモリ消費量の増加<br>ディスクアクセス速度の改善 |
