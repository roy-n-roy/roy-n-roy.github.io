...
上記のデメリットを補うために、ミラーとパリティの両方の回復性を持たせる機能が「ミラーリングによって高速化されたパリティ」です。  


### ディスク構築
本来、「ミラーリングによって高速化されたパリティ」ではWindows Server2016 Datacenter以降のOSである必要がありますが、今回はWindows10 Pro for Workstationで構築してみました。  
